...
public class JpgCardGenerator extends Frame{
	...
	private static int ImageHeight = 750;

	private int cardstatus=0;
	
	public String generateJPG(int userid) {	
		...
		if(cardstatus == 0){
			try {
				...
				cardurl=InitConfig.path+"assets/pdfcards/"+iceid+".jpg";
				FileOutputStream file = new FileOutputStream(cardurl);
				JPEGImageEncoder encoder = JPEGCodec.createJPEGEncoder(file);

			    BufferedImage bi = new BufferedImage(ImageWidth,ImageHeight,BufferedImage.SCALE_SMOOTH);
				    
			    Graphics2D pg = bi.createGraphics();
			    pg.setColor(Color.white);
				pg.fillRect(0, 0, ImageWidth,ImageHeight);
			/*Fornt Part*/
			...
			//drawing the footer
				...
				f = new Font("Arial", Font.PLAIN, 10);
				pg.setFont(f);
				pg.drawString("1) Take a print out of this Image.", 88, 600);
		...
		return "assets/pdfcards/"+iceid+".jpg";
	}
