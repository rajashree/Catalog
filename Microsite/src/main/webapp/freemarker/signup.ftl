 ...
 <#include "/freemarker/global/form-message.ftl" />
	   ...
	   <li><label><@s.text name="global.username"/>:</label><input type="text" class="txt" name="username" id="username"/><span class="sd"> <a href="#" onclick="check();">Check Availability</a></span></li>
	    ...
	    <li><label><@s.text name="global.password"/>:</label><input type="password" class="txt" name="password" id="password"/></li>
          ...
          NOTICE TO USER: PLEASE READ THIS AGREEMENT CAREFULLY. BY COPYING,	INSTALLING OR USING ALL OR 
          ANY PORTION OF THE SOFTWARE YOU ACCEPT ALLTHE TERMS AND CONDITIONS OF THIS AGREEMENT, INCLUDING, 
          IN PARTICULARTHE LIMITATIONS ON: USE CONTAINED IN SECTION2; TRANSFERABILITY IN	SECTION 4; 
          WARRANTY IN SECTIONS 6 AND 7; LIABILITY IN SECTION 8; ANDSPECIFIC PROVISIONS AND EXCEPTIONS IN SECTION 14. 
          YOU AGREE THAT THISAGREEMENT ISLIKE ANY WRITTEN NEGOTIATED AGREEMENT SIGNED BY YOU.THIS	
          AGREEMENT IS ENFORCEABLE AGAINST YOU AND ANY LEGAL ENTITY THAT OBTAINED THE SOFTWARE AND ON WHOSE BEHALF 
          IT IS USED: FOR EXAMPLE, IF APPLICABLE, YOUR EMPLOYER.IF YOU DO NOT AGREE TO THE TERMS OF THIS AGREEMENT, DO NOT USE THE SOFTWARE. VISIT http://www.mysite.com FOR TERMS OF AND LIMITATIONS ON RETURNING THE 
          SOFTWARE FOR A REFUND Do you accept the terms and conditions stated above?
          </textarea>
...
<form name="user" action="<@s.url action='signup' method="execute"/>" onsubmit="return validateRegistration();">
<div id="wrapper" class="wrapper_center">
    <div id="content">
 <#include "/freemarker/global/form-message.ftl" />
        <h2 class="block-title">User Registration</h2>
        ...
        <ul>
          <li class="register_err"><div id="error_div"  class="errormsg"></div></li>
          <li><label><@s.text name="global.firstname"/>:</label><input type="text" class="txt" value="${firstname!?html}" name="firstname" id="firstname"/><@macroFieldErrors name="firstname"/></li>
           
	   <li><label><@s.text name="global.lastname"/>:</label><input type="text" class="txt" value="${lastname!?html}" name="lastname" id="lastname"/> <@macroFieldErrors name="lastname"/></li>
           
	    <li><label><@s.text name="global.email"/>:</label><input type="text" class="txt" 
	    name="email" value="${email!?html}" id="email"/><span class="sd">    <#if registrationManager.emailValidationEnabled>will be subjected to verification</span></#if> <@macroFieldErrors name="email"/></li>
          	
	   <li><label><@s.text name="global.username"/>:</label><input type="text" class="txt" name="username" id="username"/><span class="sd"> <a href="#" onclick="check();">Check Availability</a></span><@macroFieldErrors name="username"/></li>
          
	    <li><label><@s.text name="global.password"/>:</label><input type="password" class="txt" name="password" id="password"/></li>
          <li><label>Retype Password:</label><input type="password" class="txt" name="confirmPassword" id="confirmPassword"/></li>
          <li class='terms'><textarea readonly="readonly" rows="5" cols="45" id="EULA1" name="EULA1">Software License Agreement 
          NOTICE TO USER: PLEASE READ THIS AGREEMENT CAREFULLY. BY COPYING,	INSTALLING OR USING ALL OR ANY PORTION OF THE SOFTWARE YOU ACCEPT ALLTHE TERMS AND CONDITIONS OF THIS AGREEMENT, INCLUDING, IN PARTICULARTHE LIMITATIONS ON: USE CONTAINED IN SECTION 2; TRANSFERABILITY IN	SECTION 4; WARRANTY IN SECTIONS 6 AND 7; LIABILITY IN SECTION 8; ANDSPECIFIC PROVISIONS AND EXCEPTIONS IN SECTION 14. YOU AGREE THAT THISAGREEMENT IS LIKE ANY WRITTEN NEGOTIATED AGREEMENT SIGNED BY YOU. THIS	AGREEMENT IS ENFORCEABLE AGAINST YOU AND ANY LEGAL ENTITY THAT OBTAINED THE SOFTWARE AND ON WHOSE BEHALF IT IS USED: FOR EXAMPLE, IF APPLICABLE, YOUR EMPLOYER. IF YOU DO NOT AGREE TO THE TERMS OF THIS AGREEMENT, DO NOT USE THE SOFTWARE. VISIT http://www.mysite.com FOR TERMS OF AND LIMITATIONS ON RETURNING THE SOFTWARE FOR A REFUND Do you accept the terms and conditions stated above?</textarea></li>
           <li class="agreement"> <input type="checkbox" checked="" value="true" name="agree" id="Positive"/><label>I agree <a href="#">Terms and conditions</a></label>
