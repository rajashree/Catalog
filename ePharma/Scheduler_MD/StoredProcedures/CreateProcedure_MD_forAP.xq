...
let $ind_pedship := if(exists($pedshipdata)) then (for $i in $pedshipdata return $i/pedship)[1] else <root><CustomerDea></CustomerDea></root>
let $values := tlsp:TPCustomization($ind_pedship/CustomerDea,'automatedusecase')
...
let $tot_qty := sum(for $lot_info in $shipped_lot return xs:integer(data($lot_info/quantity)))
...
declare variable $deaNumber as string external;
declare variable $sourceRoutingCode as string external;
declare variable $ndcBinNode as node()* external;
...
let $ship := tlsp:ReformatingPedShipDataNew(<root>{$final_shipdata}</root>,<root>{$ndcBinNode}</root>)

let $result := tlsp:CreateShippedPedigree_MDNew(<root>{$ship}</root>,$signerId,$deaNumber,$sourceRoutingCode)
return (for $i in $ship return if($i/name() ='status') then concat('Insufficient Quantity',',',$i/NDC/text(),',',$i/bin/text(),',',$i/LineNo/text(),',',$i/QuantityNeeded/text(),',',$i/status/text()) else (),$result)
...
let $LineNo := data($shipNode[1]/LineNo)
...
let $ndcstatus := if(exists($ndcBinNode/root/NDCBIN)) then 
		   (for $mm in $ndcBinNode/root/NDCBIN
                      return if($NDC = data($mm/NDC) and ($binNum = $mm/BinNumber)) then <result><status>Freeze</status> </result>
		           else ()) else <result><status>true</status> </result>
return 
if(data($ndcstatus/status) = 'Freeze') then <status><NDC>{$NDC}</NDC><bin>{$binNum}</bin><LineNo>{$LineNo}</LineNo><QuantityPresent></QuantityPresent><QuantityNeeded>{$tot_qty}</QuantityNeeded><ApXml>{$shipNode}</ApXml><status>Freezed</status></status>
else 
if(exists($lots)) then 
...
else (<status><NDC>{$NDC}</NDC><bin>{$binNum}</bin><LineNo>{$LineNo}</LineNo><QuantityPresent></QuantityPresent><QuantityNeeded>{$tot_qty}</QuantityNeeded><ApXml>{$shipNode}</ApXml><status>notfreezed</status></status>)
...
let $all_peds := 
  ...
  for $shipNode in $xmlNode/DS 
	...
	let $container := (<container><containerCode>{data($shipNode/CustPONumber)}</containerCode><shipmentHandle>{data($shipNode/CustPONumber)}</shipmentHandle><pedigreeHandle><serialNumber>{$docid}</serialNumber>
...
tig:create-stored-procedure("CreateShippedPedigree_MDNew_1","
import module namespace util = 'xquery:modules:util';
declare variable $xmlString as node()* external;
declare variable $signerId as string external;
declare variable $deaNumber as string external;
...
tig:create-stored-procedure("PEDSHIP_MD_1","
declare variable $shipNode as node()* external;
declare variable $signerId as string external;
declare variable $deaNumber as string external;
declare variable $sourceRoutingCode as string external;
...
let $result := tlsp:CreateShippedPedigree_MDNew_1(<root>{$ship}</root>,$signerId,$deaNumber,$sourceRoutingCode)
return (for $i in $ship return if($i/name() ='status') then concat('Insufficient Quantity',',',$i/NDC/text(),',',$i/bin/text(),',',$i/LineNo/text(),',',$i/QuantityNeeded/text(),',',$i/status/text()) else (),$result)
 
"),

tig:create-stored-procedure("InsertShippedPedigree_1","
declare variable $envNode as node()* external;
declare variable $fileName as string external;
