...
public class SQLDBConfig {
	...
	private static SQLDBConfig sqldbconf;
	
	/** The <code>log</code> is used for Log storage. */
	private static final Logger log = Logger.getLogger(SQLDBConfig.class);
...
/*		Node esbconfig = XMLUtil.parse(AppResourceUtil.getDatabaseConfigFile());
 		if(esbconfig!= null) {	 	
			...
			hostAddress = "localhost";
	 		port 		= "1433";
	 		userName 	= "sa";
	 		userPword 	= "tigerlogic";
	 		databaseName = "Pedigree_Interface";
	// 	}
	 					
	 	log.info(" SQLDBConfig all values:  "+hostAddress+" \n "+port +" \n "+ userName+" \n "+userPword+" \n ");
	}  
