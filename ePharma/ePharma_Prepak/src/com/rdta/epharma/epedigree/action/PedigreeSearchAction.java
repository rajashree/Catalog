...
public class PedigreeSearchAction extends Action{
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//Delete Functionality (Rajashree): START
		
		String pedigrees[] = request.getParameterValues("selected");
		System.out.println("lengthlengthlength: "+ pedigrees);
		if(pedigrees != null){
			int size = pedigrees.length;
			System.out.println("***********INSIDE DELETE ****************");
		        String buffer=null;
		        for(int i=0;i<size;i++){
		      	buffer="tlsp:deletePedigree('"+pedigrees[i]+"')";
		...
		if(prodNDC != null && !prodNDC.trim().equalsIgnoreCase("")&& prodNDC.length()>3){
			System.out.println("Inside If"+prodNDC);
			prodNDC = prodNDC.substring(0,4);
		}
		...
		try{	
			...
			String validate = "tlsp:validateAccess('"+sessionID+"','2.10','Insert')";
		...
		if(fromDT.equals("")&& toDT.equals("")&& trNum.equals("")  && drugName.equals("") && prodNDC.equals("") &&  tpName.equals("") ) {
			listsize = "0";
			...
			return mapping.findForward("success");
			 ...
			 populateVectors("DrugName",drugName,eltNames,eltValues);
			 ...
			 buff2.append("tlsp:getSearchResultsNew("+names+","+values+")");
		...
		for(int i=0; i<res.size(); i++){
			...
			theForm.setPedigreeId(CommonUtil.jspDisplayValue(listNode,"Record/pedigreeID"));
			log.info("pedigree ID is:"+theForm.getPedigreeId());
			System.out.println("pedigree ID is:"+theForm.getPedigreeId());
			
			theForm.setDrugName(CommonUtil.jspDisplayValue(listNode,"Record/drugName"));
			log.info("DrugName is:"+theForm.getDrugName());
			System.out.println("DrugName is:"+theForm.getDrugName());
			
			theForm.setManufacturer(CommonUtil.jspDisplayValue(listNode,"Record/manufacturer"));
			log.info("Manufacturer is:"+theForm.getManufacturer());
			System.out.println("Manufacturer is:"+theForm.getManufacturer());
			
			theForm.setNdc(CommonUtil.jspDisplayValue(listNode,"Record/NDC"));
			log.info("Ndc is:"+theForm.getNdc());
			System.out.println(" Ndc is:"+theForm.getNdc());
			
			theForm.setLotNo(CommonUtil.jspDisplayValue(listNode,"Record/lotNo"));
			log.info("lotNo is:"+theForm.getLotNo());
			System.out.println("lotNo is:"+theForm.getLotNo());
			
			theForm.setQuantity(CommonUtil.jspDisplayValue(listNode,"Record/quantity"));
			log.info("quantity is:"+theForm.getQuantity());
			System.out.println("quantity is:"+theForm.getQuantity());
			
			
			
			theForm.setEnvelopeId(CommonUtil.jspDisplayValue(listNode,"Record/envelopID"));
			log.info("Envelope ID is:"+theForm.getEnvelopeId());
			System.out.println("Envelope ID is:"+theForm.getEnvelopeId());
			
			theForm.setDataRcvd(CommonUtil.jspDisplayValue(listNode,"Record/dateRecieved"));
			System.out.println("dataRcvd is:"+theForm.getDataRcvd());
			log.info("dataRcvd is:"+theForm.getDataRcvd());
			
			theForm.setTrdPrtnr(CommonUtil.jspDisplayValue(listNode,"Record/tradingPartner"));
			log.info("Trading partner name is:"+theForm.getTrdPrtnr());
			System.out.println("Trading partner name is:"+theForm.getTrdPrtnr());
			
			theForm.settrNum(CommonUtil.jspDisplayValue(listNode,"Record/transactionNumber"));
			log.info(" Transaction Number is :"+theForm.gettrNum());
			System.out.println("dataRcvd is:"+theForm.gettrNum());
			
			theForm.setStatus(CommonUtil.jspDisplayValue(listNode,"Record/status"));
			log.info("status is:"+theForm.getStatus());
			System.out.println("Transaction Number is :"+theForm.getStatus());
			
			theForm.setCreatedBy(CommonUtil.jspDisplayValue(listNode,"Record/createdBy"));
