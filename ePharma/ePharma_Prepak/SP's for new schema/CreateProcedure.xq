...
"
 ...
 <Record>
 {
 <pedigreeID>{$ped/initialPedigree/DocumentInfo/serialNumber/text()}</pedigreeID>,
  <dateRecieved>{data($ped/initialPedigree/transactionInfo/transactionDate)}</dateRecieved>,
  <tradingPartner>{$ped/initialPedigree/transactionInfo/senderInfo/businessAddress/businessName/text()}</tradingPartner>,
  <transactionNumber>{$ped/initialPedigree/transactionInfo/transactionIdentifier/identifier/text()}</transactionNumber>,
  <docURI>{$docURI}</docURI>,
<lotNo>{$ped/initialPedigree/itemInfo/lot/text()}</lotNo>,
<NDC>{$ped/initialPedigree/productInfo/productCode/text()}</NDC>,
<manufacturer>{$ped/initialPedigree/productInfo/manufacturer/text()}</manufacturer>,
<drugName>{$ped/initialPedigree/productInfo/drugName/text()}</drugName>,
<quantity>{$ped/initialPedigree/itemInfo/quantity/text()}</quantity>
 }
 </Record>	
 };
 ""};
 ...
 declare variable $g_search_elts as node()
		{<Search>
		   <Scenarios>	
			<Scenario>	
				<Collections>
					<Collection>tig:///ePharma/PaperPedigree</Collection>	
				</Collections>
		   		<Key>
					<Name>FromDate</Name>
					<RootSelection>
					    <Query>
						<RootPrefix>Yes</RootPrefix>
						<Path>initialPedigree/transactionInfo/transactionDate/text()</Path>
						<Operator>&gt;=</Operator>
						<Value>$UserVal</Value>
					    </Query>	
					</RootSelection>
				...
				<Key>
					...
					<RootSelection>
					    <Query>
						<RootPrefix>Yes</RootPrefix>
						<Path>substring(data(initialPedigree/productInfo/productCode),0,4)</Path>
						<Operator>=</Operator>
						<Value>$UserVal</Value>
					    </Query>	
					</RootSelection>
				...
				<Key>
					...
					<RootSelection>
					    <Query>
						<RootPrefix>Yes</RootPrefix>
						<Path>initialPedigree/productInfo/drugName</Path>
						<Operator>=</Operator>
						<Value>$UserVal</Value>
					    </Query>	
					</RootSelection>
				...
				<Key>
					...
					<RootSelection>
					    <Query>	
						<RootPrefix>Yes</RootPrefix>		
						<Path>initialPedigree/transactionInfo/transactionIdentifier/identifier</Path>
						<Operator>=</Operator>
						<Value>$UserVal</Value>
					    </Query>	
					</RootSelection>
				...
				<Key>
					...
					<RootSelection>
					    <Query>
						<RootPrefix>Yes</RootPrefix>			
						<Path>initialPedigree/transactionInfo/senderInfo/businessAddress/businessName</Path>
						<Operator>=</Operator>
						<Value>$UserVal</Value>
					    </Query>	
					</RootSelection>
 ...
 return concat( 
  ...
  for $col_uri in $g_search_elts//Scenario[$index]/Collections/Collection
  return local:getQuery($col_uri/text(), $root_where_clause ,$root_node, $index = 1 ,$recvd_local_fn_1 ,$recvd_local_fn_3 ,$count_elts )
...
for $i in collection('tig:///ePharma/PaperPedigree')
where $i/initialPedigree/DocumentInfo/serialNumber = $pedigreeId
return tig:delete-document(document-uri($i))
")
