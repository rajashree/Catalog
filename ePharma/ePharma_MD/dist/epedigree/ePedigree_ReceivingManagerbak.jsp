...
String dispNum  = (String)request.getAttribute("dispNum");
String trNum  = (String)request.getAttribute("trNum");
String SSCC  = (String)request.getAttribute("SSCC");
String apndocId  = (String)request.getAttribute("apndocId");
String invoiceNum = (String)request.getAttribute("invoiceNum");
String tpName = (String)request.getAttribute("tpName");
String prodNDC = (String)request.getAttribute("prodNDC");
//if(SSCC == null) { SSCC = "";}
...
if (tp_company_nm.equals("")) {
	query = "for $i in collection('tig:///CatalogManager/TradingPartner')/TradingPartner ";
	...
	tpNames = "<option value ='"+tp_company_nm+"'>"+tp_company_nm+"</option>";
}
										...
										<tr>
											...
											<FORM action="/ePharma/dist/epedigree/ePedigree_ReceivingManager.do" method="post">
													...
													<tr>
														...
														<td align="left">
															<!-- Dashboard Start -->
															<%if(request.getAttribute("screenEnteredDate")!=null){%>
															## Pedigrees Dated:<%=screenEnteredDate%>
															<%}else{
															boolean conditionAdded=false;%>
															<%if(request.getAttribute("lotNum")!=null){%>
														     ## LOT NUMBER:<%=lotNum%>
															<%}else{}
															
															if(request.getAttribute("tpName")!=null){
															if(!conditionAdded)%>
															 ## TRADING PARTENER NAME:<%=tpName%>
															<%conditionAdded=true;%>
															<%}else{  }
															if(request.getAttribute("trNum")!=null){
															if(!conditionAdded)%>
															 ## TRANSACTION NUMBER:<%=trNum%>
															 <%conditionAdded=true;%>
															  <%}else{  }
															if(request.getAttribute("dispNum")!=null){
															if(!conditionAdded)%>
															##  Despatch Advice NUMBER:<%=dispNum%>
															 <%}else{  }
															if(request.getAttribute("SSCC")!=null){
															if(!conditionAdded)%>
															 ## SSCC NUMBER:<%=SSCC%>
															 <%}else{  }
															if(request.getAttribute("apndocId")!=null){
															if(!conditionAdded)%>
															 ## DOCUMENT ID:<%=apndocId%>
															 <%}else{  }
															if(request.getAttribute("prodNDC")!=null){
															if(!conditionAdded)%>
															 ## NDC NUMBER:<%=prodNDC%>
															 <%conditionAdded=true;%>
															 <%}else{  }
															if(request.getAttribute("invoiceNum")!=null){
															if(!conditionAdded)%>
															...
															<table id="Table12" cellSpacing="1" cellPadding="0" width="100%" align="center" border="0">
																...
																<tr class="tableRow_Header">
																		...
																		<TD class="type-whrite" align="center">Num Of Pedigrees</TD>
																		<td class="type-whrite" noWrap align="center">Products In Shipment</td>
																	...
																	<tr class="tableRow_Off">
																		...
																		<td class='td-content'><A href='ePedigree_Manager_Reconcile.jsp?pedid=<bean:write name="rcvng" property="pedigreeNum"/>&pagenm=pedigree&tp_company_nm=<%=tp_company_nm%>&dvs=<bean:write name="rcvng" property="transactionType"/>'>
																			<bean:write name="rcvng" property="pedigreeNum"/>
																		</td>	
																		<td class='td-content'><A href='ePedigree_ViewOrder.jsp?pagenm=epedigree&tp_company_nm=&trNum=<bean:write name="rcvng" property="trNum"/>&trType=PO' >
																			<bean:write name="rcvng" property="trNum"/>
																		</td>
																		<td>
																		...
																		<td>
																			<bean:write name="rcvng" property="productsInShipment"/>
																		</td>
																		<td>
