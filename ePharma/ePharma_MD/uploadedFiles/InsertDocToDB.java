...
public class InsertDocToDB {
	...
	public static String CreateManualPedigreeEnvelopeForMD(String xmlString,String signerid,String deaNumber,String sourceRoutingCode,String fileName) throws Exception{
		// TODO Auto-generated method stub
		String qtoInsert = "";
		...
		try {
			...
			String query = "tlsp:InsertShippedPedigree("+XMLUtil.convertToString(ele,true)+",'"+fileName+"')";
			envId = queryRunner.returnExecuteQueryStringsAsString(query);
			}
			return envId; 
	...
	public static void CreatePedigreeEnvelopeForDropShip(String xmlString, String signerid, String deaNumber, String sourceRoutingCode, String[] mail, String fileName) throws Exception{
		// TODO Auto-generated method stub
		String qtoInsert = "";
		qtoInsert = "tlsp:CreateShippedPedigreeForDropShip_MD("+xmlString.toString().replaceAll("'","&apos;")+",'"+signerid+"','"+deaNumber+"','"+sourceRoutingCode+"')";
		log.info("Query for Create Pedigree : "+qtoInsert);
		qtoInsert = qtoInsert.replaceFirst("xmlns:pen", "xmlns");
		try {
			//List data = queryRunner.returnExecuteQueryStrings(qtoInsert);
			...
			for(int m=0;m<data.size();m++){
				String dataString = data.get(m).toString();
				log.info("REsult Envelope NOde Before IF : "+dataString);
				 if(dataString.equalsIgnoreCase("Insufficient Quantity")){    


					 SendDHFormEmail.sendMailToSupport(mail[0], mail[1] , mail[2],
						"ePharma Morris & Dickson Error","Insufficient Quantity",mail[3], mail[4] );   
