...
let $config := (for $i in collection('tig:///ePharma_MD/PedigreeTradingPartner')/PedigreeTradingPartner
...
let $ccode := for $i in $config where $i/element = 'containerCode' return data($i/value)
let $shipment := for $i in $config where $i/element = 'shipmentHandle' return data($i/value)
return 
<result>
{
...
let $result := tlsp:CreateManualShippedPedigree_MDNew(<root>{$final_shipdata}</root>,$signerId,$deaNumber,$sourceRoutingCode)
return $result
