...
public class Helper {
	...
	*/
    ...
    public boolean writeToTL(InputStream fileIS, String locationID,Statement statement)
    {
        ...
        String insXML = "<Location>";        
        insXML = insXML + "<LocIMG>"+ "{binary {$1}}"+"</LocIMG>";
        insXML = insXML +"<ID>"+locationID+"</ID>";        
        insXML = insXML +"</Location>";
        String FullQuery = "declare binary-encoding none;";
        FullQuery = FullQuery + "tig:insert-document( 'tig://root/EAGRFID/ProductImage/', "+insXML+" )";
        System.out.println("Img :"+FullQuery);
    ...
    public boolean writePrdImgToTL(InputStream fileIS, String productID,Statement statement)
    {
        try {
        	...
        	bfr.append("<Product>");        
            bfr.append("<ProdIMG>"+ "{binary {$1}}"+"</ProdIMG>");
            bfr.append("<ID>"+productID+"</ID>");        
            bfr.append("</Product>");  
        
        String FullQuery = "tig:insert-document( 'tig://root/EAGRFID/ProductImage/', "+bfr.toString()+" )";
        executeStatementStream(FullQuery, fileIS,statement);
