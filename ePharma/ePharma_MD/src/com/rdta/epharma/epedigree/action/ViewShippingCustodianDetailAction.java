 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class ViewShippingCustodianDetailAction extends Action {
	
	private static Log log = LogFactory.getLog(ViewShippingCustodianDetailAction.class);
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try{
			...
			log.info("sessionID in ViewShippingCustodianDetailAction :"+sessionID);
			...
			log.info("Inside Action ViewShippingCustodianDetailAction....... ");
			log.info("Inside Action execute of ViewShippingCustodianDetailAction....");
		
			String APNID = request.getParameter("pedId");
			log.info("The APNID inside ViewShippingCustodianDetailAction is :"+APNID);
		
			String tp_company_nm = request.getParameter("tp_company_nm");
			log.info("The tp_company_nm inside ViewShippingCustodianDetailAction is :"+tp_company_nm);
		
			String pagenm = request.getParameter("pagenm");
			log.info("The pagenm inside ViewShippingCustodianDetailAction is :"+pagenm);
		
			Collection colln= new ArrayList();
	
			String name = request.getParameter("name");
			log.info("The manufacturer name in ViewShippingCustodianDetailAction is :"+name);
			...
			buffer.append("<td class='type-whrite'>Transaction Date : <FONT color='#ffff00'><STRONG>{data($b/DateTime)}</STRONG></FONT></td> ");
			...
			log.info("The query in ViewShippingCustodianDetailAction is :"+buffer.toString());
		
			String topLine = queryRunner.returnExecuteQueryStringsAsString(buffer.toString());
		
			request.setAttribute("topLine", topLine);		
			log.info("Result after executing the query for topLine in ViewShippingCustodianDetailAction is :"+topLine);
		
			buffer1 = new StringBuffer();
		
			buffer1.append("for $b in collection('tig:///ePharma/APN')/APN[DocumentId='"+APNID+"'] ");
			buffer1.append("where $b/Pedigrees/Pedigree/Custody/Name='"+name+"' ");
			...
			log.info("Before executing the query in ViewShippingCustodianDetailAction to get custodian details......");
			String custodianDetails = queryRunner.returnExecuteQueryStringsAsString(buffer1.toString());
			log.info("The custodian details is :"+custodianDetails);
			
			request.setAttribute("custodianDetails", custodianDetails);
			
			log.info("Before returning to success.........from ViewShippingCustodianDetailAction.......");
			
		}catch(Exception ex){
			ex.printStackTrace();
			log.error("Error inside ViewShippingCustodianDetailAction class......."+ex);
