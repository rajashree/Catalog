...
public class InsertDocToDB {
	...
	public static void CreatePedigree(String RefNum,String orderNo,String trType,String flag,String pedID,String sessionID) throws Exception{
		...
		if(trType.equalsIgnoreCase("PurchaseOrderNumber")){
			qtoInsert = "tlsp:CreateShippedPedigreeForOrders_MD('"+RefNum+"','"+orderNo+"','"+trType+"','"+flag+"','"+pedID+"','"+sessionID+"')";
		}else
			qtoInsert = "tlsp:CreateShippedPedigreenew_MD('"+RefNum+"','"+trType+"','"+flag+"','"+pedID+"','"+sessionID+"')";
	...
	public void createReceivedPedigree(String pedId,String sessionId) throws Exception{
		...
		qtoInsert = "tlsp:CreateReceivedPedigreeForPedigreesNew_MD('"+pedId+"','"+sessionId+"')";
		...
		try {
			...
			String query = "tlsp:PedigreeBankUpdateAfterReceivedPedigree_MD('"+pedigree+"')";
			queryRunner.executeQuery(query);
	...
	public static void CreatePedigreeEnvelopeForMD(String xmlString) throws Exception{
		// TODO Auto-generated method stub
		String qtoInsert = "";
		qtoInsert = "tlsp:CreateManualShippedPedigree_MDNew('"+xmlString+"')";
		System.out.println("Query for Create Pedigree : "+qtoInsert);
		qtoInsert = qtoInsert.replaceFirst("xmlns:pen", "xmlns");
		
		try {
			String dataString = queryRunner.returnExecuteQueryStringsAsString(qtoInsert);
			//List data = queryRunner.returnExecuteQueryStrings(qtoInsert);
			//String dataString = (String) data.get(0);
			System.out.println(dataString);
			dataString = dataString
			...
			String query = "tig:insert-document('tig:///ePharma_MD/ShippedPedigree',"+XMLUtil.convertToString(ele,true)+")";
			String url = queryRunner.returnExecuteQueryStringsAsString(query);
			
			String sp = "tlsp:CreateManualShippedPedigreeSign_MD('"+url+"') ";
			System.out.println("The URL is :"+ url);
			queryRunner.executeQuery(sp);
