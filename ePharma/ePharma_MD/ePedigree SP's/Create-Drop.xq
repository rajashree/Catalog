...
<quantity>{data($e/shippedPedigree/itemInfo/quantity )}</quantity>,
...
(:Quick find options Resultdqueries sp:)
...
let $e := ( for $i in collection(concat('tig:///ePharma/',$catalogName))//documentInfo[ serialNumber = $serualId]
	   return $i/parent::*)
return <output>{
...
declare variable $envelopeNumber as string external;
declare variable $serialNumber as string external;
for $e in collection('tig:///ePharma/ShippedPedigree')/PedigreeEnvelope[serialNumber = $envelopeNumber ]/pedigree
where $e/shippedPedigree/documentInfo/serialNumber=$serialNumber
return <out><previousProducts>{$e/shippedPedigree[1]/repackagedPedigree/previousProducts}</previousProducts><productInfo>{$e/shippedPedigree[1]/repackagedPedigree/productInfo}</productInfo></out>"),
...
let $pedID := $pedNode//PedigreeData/pedigreeID/text()

let $sessionID := tlsp:GetSessionID($signerID)
return (
tlsp:CreateReceivedPedigreeForPedigrees($pedID,$sessionID),
tlsp:CreateSignToReceivedPedigree($pedID),
tlsp:InsertAndChangePedigreeStatus($xmlString,'Received And Authenticated')
)
"),
...
let $pedNode := tlsp:StringToNode($xmlString)
let $userid := $pedNode//PedigreeData/signerID/text()
let $pedId := $pedNode//PedigreeData/pedigreeID/text()
let $time := fn:current-dateTime()
...
let $pedNode := tlsp:StringToNode($xmlString)
let $productNode := $pedNode/SQLXMLData/PedigreeData/ProductInfo
let $itemNode := $pedNode/SQLXMLData/PedigreeData/ItemInfo
let $pedigree := tlsp:GetRPORInitialPedigreeDetails($pedId)
let $pedigreeType := $pedigree/PedigreeType
let $signerInfo := for $y in collection('tig:///EAGRFID/SysUsers')/User
where $y/UserID = $pedNode/SQLXMLData/PedigreeData/signerId/text()
return <signerInfo><name>{data($y/FirstName)}</name><title>{data($y/UserRole)}</title><telephone>{data($y/Phone)}</telephone>
<email>{data($y/Email)}</email><url>{data($y/Email)}</url>
</signerInfo>
return
...
"),
tig:drop-stored-procedure("createInitialPedigreeStructure"),
...
declare variable $faxName as string external;
update
for $i in collection('tig:///ePharma/ReceivedFax')/InboundPostRequest/FaxControl[FaxName = $faxName]
replace value of $i/Status with 1
"),
