  <head>
	...
	<style type="text/css">@import "<@s.url value='/theme/tree/css/dhtmlxtree.css'/>";</style>
	...
	<style type="text/css">@import "<@s.url value='/theme/calendar/css/smoothness.datepick.css'/>";</style>
	...
	<script type='text/javascript'>
	 ...
	 var lastClickName;
	 var isSwitch=false;
	 var featuresNamedArray = new Array();
	...
	function resetSavePanel(){
		...
		dwr.util.setValue("applicationdefault",false);
	...
	populateFeaturesNamedArray();
	function populateFeaturesNamedArray(){
	    userDwrService.getAllFeatureList(function(data) {
   	 		for (i=0;i<data.length;++ i){
				featuresNamedArray[data[i].id]=data[i].name;
			}
   	 		//alert(featuresNamedArray[670]);		
   	 	});
	}
	//Load Features
	...
	function loadAdminSettings(){
		userDwrService.isAdminUser("${request.remoteUser}",
	    	function(data) {
				if(data)
					dwr.util.byId("admin-settings").style.display = "block";
			});
	...
	function handleSearchDetails(data){
    	...
    	alert("data.created"+data.created);
    	alert("data.sentimentIds"+data.sentimentIds);
    	alert("data.volume"+data.volume);
    	alert("data.buzz"+data.buzz);
    	alert("data.productIds"+data.productIds);
    	alert("data.featureIds"+data.featureIds);
    	alert("data.orientation"+data.orientation);
    	alert("data.sourceTypes"+data.sourceTypes);
    	...
    	*/
		...
		if(data){
			...
			sourcefilter_values=data.sourceTypes;
			resetFilters();
			...
			if(!endDate)
			endDate = "";
			//alert("isBuzz::"+isBuzz+"product_ids:::"+product_ids+"feature_ids:::"+feature_ids+"sentimentfilter_values:::"+sentimentfilter_values+"sourcefilter_values:::"+sourcefilter_values+"startDate::::"+startDate+"endDate:::"+endDate);
	...
	function getSearchDetails(sid){
		...
		userDwrService.isApplicationDefaultSearch(sid, handleApplicationDefaultSearchDetails);
		userDwrService.getSavedSearch(sid,handleSearchDetails);
		
	}
	
	function handleApplicationDefaultSearchDetails(data){
		if(data)
			dwr.util.setValue("applicationdefault",true);
		else
			dwr.util.setValue("applicationdefault",false);
	}
	...
	function handleReviewData(data) {
		...
		//alert("handleReviewData"+data);
			...
			if(isBuzz == "true"){
				...
				var seriesColor1="579EA4";
				var seriesColor2="D25244";
				var seriesColor3="949A4B";
				var seriesColor4="D08C60";
				var seriesColor5="4B9E9E";
				var seriesColor6="7c7c7c";
				...
				var seriesColor1="279A01";
				var seriesColor2="ff0000";
				var seriesColor3="abcdef";
				var seriesColor4="ffff00";
			...
			if(!isSwitch)
				...
				for(var j=0;j<data.products.length;j++){
					...
					for (var i=0;i<data.features.length;i++){
						...
						//alert((result[feature+'_name']?result[feature+'_name']:featuresNamedArray[feature]));
					...
					//	alert("product_name"+result.product_name);
						...
						dataSetXml1=dataSetXml1+" <set value='"+ ((result[accessName1]) ? result[accessName1] : 0)+"'  link='javascript:getPosts(\""+result.product_id+"\",\""+feature+"\",\""+result.product_name+"\",\""+(result[feature+'_name']?result[feature+'_name']:featuresNamedArray[feature])+"\",\"1\")' />";
						dataSetXml2=dataSetXml2+" <set value='"+((result[accessName2]) ? result[accessName2] : 0)+"'  link='javascript:getPosts(\""+result.product_id+"\",\""+feature+"\",\""+result.product_name+"\",\""+(result[feature+'_name']?result[feature+'_name']:featuresNamedArray[feature])+"\",\"2\")' />";
						dataSetXml3=dataSetXml3+" <set value='"+((result[accessName3]) ? result[accessName3] : 0)+"'  link='javascript:getPosts(\""+result.product_id+"\",\""+feature+"\",\""+result.product_name+"\",\""+(result[feature+'_name']?result[feature+'_name']:featuresNamedArray[feature])+"\",\"3\")' />";
						dataSetXml4=dataSetXml4+" <set value='"+((result[accessName4]) ? result[accessName4] : 0)+"'  link='javascript:getPosts(\""+result.product_id+"\",\""+feature+"\",\""+result.product_name+"\",\""+(result[feature+'_name']?result[feature+'_name']:featuresNamedArray[feature])+"\",\"4\")' />";
						if(isBuzz == "true"){
		...
		//alert(xmlString);
			var chartObj = getChartFromId("chartBarId");
				...
				//document.getElementById("chartxml").value=xmlString;
			...
			if(!isSwitch){
				for (var i=0;i<products.length;i++){	
					if(limit < 58/features.length){
						for (var j=0;j<features.length;j++){
							catString=catString+"<category  label=\"&lt;a href=&quot;javascript:dodrilldown('"+products[i].product_id+"','"+features[j]+"','"+products[i].product_name+"',false)&quot;&gt;"+products[i].product_name+"&lt;/a&gt;\" rlabel=\"&lt;a href=&quot;javascript:dodrilldown('"+products[i].product_id+"','"+features[j] +"\','"+(products[i][features[j]+"_name"]?products[i][features[j]+"_name"]:featuresNamedArray[features[j]])+"',true)&quot;&gt;"+(products[i][features[j]+"_name"]?products[i][features[j]+"_name"]:featuresNamedArray[features[j]])+"&lt;/a&gt;\" id='"+features[j]+"' />";
				    	}
						limit++;
					}
				}
			    ...
			    for (var j=0;j<features.length && j < 58/features.length;j++)
			    	for (var i=0;i<products.length;i++)
						...
						catString=catString+"<category  toolText='"+(products[i][features[j]+"_name"]?products[i][features[j]+"_name"]:featuresNamedArray[features[j]])+"' label=\"&lt;a href=&quot;javascript:dodrilldown('"+products[i].product_id+"','"+features[j]+"','"+(products[i][features[j]+"_name"]?products[i][features[j]+"_name"]:featuresNamedArray[features[j]])+"',false)&quot;&gt;"+(products[i][features[j]+"_name"]?products[i][features[j]+"_name"]:featuresNamedArray[features[j]])+"&lt;/a&gt;\"   rlabel=\"&lt;a href=&quot;javascript:dodrilldown('"+products[i].product_id +"','"+features[j]+"','"+products[i].product_name+"',true)&quot;&gt;"+products[i].product_name+"&lt;/a&gt;\"   id='"+features[j]+"'/>";			    
			}
			
			catString=catString+"</categories>";
		//	alert(catString);	
		//	document.getElementById("category").value=catString;
			return catString;
...
*/		
   	...
   	function genCategoryXML(products , features){
			...
			if(!isSwitch){
			    ...
			    for (var j=0;j<features.length && j < 58/products.length;j++)
			    	for (var i=0;i<products.length;i++)
	...
	function dodrilldown(pid,fid,name,isRight){
		lastClickName=name;
	...
	dwr.util.byId("block3").style.display ="none";
	dwr.util.byId("block2").style.display ="none";
	...
	function getPostData(postId,itemNo){
		dwr.util.byId("post_data"+postId).style.display = "block";
		dwr.util.byId("snippet_data"+postId).style.display = "none";
		dwr.util.byId(("postdetaillink"+itemNo)).className="active";
		dwr.util.byId(("snippetlink"+itemNo)).className="";
	}
	...
	function handleSnippetsData(data){
		//alert(":::::::::postdetaillink"+tempItemNo);
		dwr.util.byId(("postdetaillink"+tempItemNo)).className="";
		dwr.util.byId(("snippetlink"+tempItemNo)).className="active";
	...
	function handlePostData(data){
		...
		if(data){
			...
			for(var j=0;j<products.length;j++){
			 ...
			 //temp+="<div class='posts'><h3>"+products[j].product+" : "+products[j].title+"<span class='post-link'><a href='javascript:void(0);' onclick='getAllSnippets(\""+products[j].id+"\",\""+j+"\")' >Snippet</a> | <a class='active' onclick='getPostData(\""+products[j].id+"\",\""+j+"\")'  href='javascript:void(0);'>Details</a></span></h3><div id='post_data"+products[j].id+"' class='post_data'><p>"+products[j].post+"</p></div><div id='snippet_data"+products[j].id+"' style='display:none'></div><div class='products-name'><span style='font-weight: bold;'>Product : </span>"+products[j].product+"<a href='javascript:void(0);'  onclick='loadPost_SendFeedback("+j+",\"div"+j+"\");'  class='feedback-post'>Feedback on this post</a></div>"+sentiment_html+"<div class='posts_foot'> <a href='#' class='site'>"+products[j].url+"</a><span class='date-posted'>"+products[j].date+"</span><a href='#' class='report-spam'>Report Spam</a>  </div></div>";
			 temp+="<div class='posts'><span class='posts_title'><h3>"+products[j].product+" : "+products[j].title+"<span class='post-link'><a id='snippetlink"+j+"' href='javascript:void(0);' onclick='getAllSnippets(\""+products[j].id+"\",\""+j+"\")' >Snippet</a> | <a  id='postdetaillink"+j+"'  class='active' onclick='getPostData(\""+products[j].id+"\",\""+j+"\")'  href='javascript:void(0);'>Details</a></span></h3></span><div id='post_data"+products[j].id+"' class='post_data'><p>"+products[j].post+"</p></div><div id='snippet_data"+products[j].id+"'  class='snippet_data' style='display:none'></div><div class='posts_rightpanel'><p class='posts_productname'><span style='font-weight: bold;'>Product : </span>"+products[j].product+"</p><p class='posts_url'><a href='"+products[j].url+"' target='_blank' class='site'>"+products[j].url.slice(0,25)+"...</a></p><p class='posts_date'>"+products[j].date+"</p><p class='posts_feedback'><a href='javascript:void(0);'  onclick='loadPost_SendFeedback("+j+",\"div"+j+"\");'  class='feedback-post'>Feedback on this post</a></p></div><div class='posts_foot'>"+sentiment_html+"</div></div>"
		    }
		    var startIndex;
			var totalIndex;
			...
			var pageHtml= genPaginationHtml(startIndex,totalIndex+1);
	...
	function genPaginationHtml(start,total){
		...
		if(start == 1){
		   	pageHtml=pageHtml +  "<a class='bb disabled'><<</a><a class='b disabled'><</a>";
		}
		for(var i=start; i<start+ 1 && i<= total; i++)
			pageHtml=pageHtml+"<span>Page </span><a class='nums' onclick=\"getNextPosts('"+i+"')\" href='javascript:void(0);'>"+i+"</a><span> of "+total+"</span>";
		if(start < total)
			...
			pageHtml=pageHtml+"<a class='f disabled'>></a><a class='ff disabled'>>></a>";
	...
	function saveReport(){
		...
		if(!isEditSearch){
			...
			var isDefault=false;
			var isApplicationDefault=false;
			if(dwr.util.getValue("default"))
				isDefault=true;
				
			if(dwr.util.getValue("applicationdefault"))
			...
			tempChartTitle = title;
			//alert("isDefault ::"+isDefault+"orientation ::"+orientation+"Start Dat"+startDate+"end date ::"+endDate+"title"+title+"description"+description+"isBuzz"+isBuzz+"isVolume"+isVolume+"product_ids"+product_ids+"feature_ids"+feature_ids+"sentimentfilter_values"+sentimentfilter_values+"sourcefilter_values"+sourcefilter_values);
			
			
			if(!waitingLock ){
				waitingLock=true;
					userDwrService.saveSearch(title,description,username,isBuzz, isVolume, product_ids, feature_ids, sentimentfilter_values, sourcefilter_values,startDate, endDate, orientation,true,isDefault,isApplicationDefault, handleSaveSearchData);
			}	
		}else{
			if(!lastResult){	 		  
				alert("No Search to Save");	
			...
			tempChartTitle = title;
			var orientation =(isSwitch == 0)?"Products":"Features";
			//alert("editSearchId :::"+editSearchId+"isDefault ::"+isDefault+"orientation ::"+orientation+"Start Dat"+startDate+"end date ::"+endDate+"title"+title+"description"+description+"isBuzz"+isBuzz+"isVolume"+isVolume+"product_ids"+product_ids+"feature_ids"+feature_ids+"sentimentfilter_values"+sentimentfilter_values+"sourcefilter_values"+sourcefilter_values);
			if(!waitingLock ){
				waitingLock=true;
					userDwrService.updateSearch(editSearchId,title,description,username,isBuzz, isVolume, product_ids, feature_ids, sentimentfilter_values, sourcefilter_values,startDate, endDate, orientation,true,isDefault,isApplicationDefault, handleUpdateSearchData);
			}
		}
	}
	...
	function handleUpdateSearchData(data){
		...
		if(data == true){
			...
			temp="";
			dwr.util.byId("savebutton").innerHTML = "Save";
			dwr.util.byId("chartTitle").innerHTML = tempChartTitle
			loaduserSavedSearch(defaultsearchstart,defaultsearchcount);		
		}else{
	...
	//Send Feedback related functions
	...
	function sendFeedback(){
		...
		if(dwr.util.byId("comments").value){
			if(!waitingLock){
				waitingLock=true;			
			   // alert("dwr.util.getValue('username') :::"+dwr.util.getValue("username")+"dwr.util.byId(comments).value:::"+dwr.util.byId("comments").value+"currentPost.url:::"+currentPost.url+"currentPost.title:::"+currentPost.title+"currentPost.date:::"+currentPost.date+currentPost.product+"::::"+currentPost.snippet+":::::"+metricName+"::::"+sentimentNames+":::::"+buzzNames+"::::"+displayTypeName+":::::"+prodNames+"::::"+featureNames);
			    userDwrService.sendFeedback(dwr.util.getValue("username"),dwr.util.byId("comments").value,currentPost.url,currentPost.title,currentPost.date,currentPost.product,metricName,sentimentNames,buzzNames,displayTypeName,prodNames,featureNames,handleSendFeedback);	
			}	
		}
	}
      		   		  ...
      		   		  <span class="clear-all"><a href="javascript:void(0);" onclick="newSearch();"   onmouseover="Tip('<@s.text name="label.clear.all.tooltip"/>')" onmouseout="UnTip()"  ><@s.text name="label.clear.all"/></a></span>
      		   		   <div tabindex="0" class="Accordion" id="Acc7">
							  <div class="AccordionPanel AccordionPanelClosed">
								  ...
								  <div id="block2" style=" overflow:auto !important; height:220px !important; width:200px !important; margin-left:12px; border:1px solid Silver;display: none;"  class="AccordionPanelContent" >
								     </div>
							  </div>
							  <div class="AccordionPanel AccordionPanelClosed">
							      ...
							      <div id="block3"  style=" overflow:auto !important; height:220px !important; width:235px !important; margin-left:12px; border:1px solid Silver;display: none;" class="AccordionPanelContent" >
			        	 ...
			        	 <div id="metric_buzz">
					        <ul class="sec2">
						     ...
						     <ul class="sec3">
        						...
        						<ul class="date_range">
<li class="label-group"> Date Range </li>
<li><span>Start </span><input type="text" id="startDate"/></li>
<li><span>End </span> <input type="text" id="endDate"/></li>
</ul>
      		...
      		<#-- <input type="text" value="" id="chartxml"/>
    	...
    	<div id="savereport-accordian" class="bock-rep">
         	 ...
         	 <div class="breadcrumbs" id="breadcrumbs"></div>
         	  ...
         	  <div id="chartBarDiv" style="height: 400px;width: 640px;overflow: auto; float:left;" ></div>
		    	<div class="axis-dtls axis-dtls-switch ">
			 ...
			 <ul class="block-foot">
			       ...
			       <li id="totSentiments"></li>	      
		             ...
		             <li><a href="javascript:void(0);" class="print" onmouseover="Tip('<@s.text name="label.chart.print.tooltip"/>')" onmouseout="UnTip()"  onclick="javascript:printChart();"><@s.text name="label.print"/></a></li>
		          	<li class="last"><div id="savereport-header" ><a onclick="loadSaveReportDiv()" href="javascript:void(0);" class="save" onmouseover="Tip('<@s.text name="label.chart.save.tooltip"/>')" onmouseout="UnTip()"  ><div id="savebutton">Save</div></a></div></li>
		        </ul> 
			  </div>
			  <div id="block4">
				...
				<div id="savereport-content" style="display: none;" class="description">
				     <h3><@s.text name="label.save.search"/></h3>
							...
							<label class="default-label" style="margin-top:20px !important;">
	           				 	<input name="default" id="default" value="true" type="checkbox">		         
	            			    <@s.text name="label.default.search"/>
	            			</label>
	            			<div id="admin-settings" style="display:none">
		            			<label class="default-label" style="margin-top:20px !important;">
		           				 	<input name="applicationdefault" id="applicationdefault" value="false" type="checkbox">		         
		            			    <@s.text name="label.applicationdefault.search"/>
		            			</label>
		            		</div>
