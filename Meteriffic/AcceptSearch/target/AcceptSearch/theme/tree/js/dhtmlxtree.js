...
function dhtmlXTreeObject(htmlObject,width,height,rootId){
 ...
 if(this.allTree.offsetWidth>20)
	 this.selectionBar.style.width=(this.allTree.offsetWidth-20)+"px";
...
{
 case "SELECT": 
	 this.selectItem(itemId,false);break;
 case "CALL": 
	 this.selectItem(itemId,true);break;
 case "CHILD":
	 Nodes[Count].XMLload=0;break;
 case "CHECKED": 
	 if(this.XMLloadingWarning)
 ...
 case "HCHECKED":
	 ...
	 break;
 case "OPEN": Nodes[Count].openMe=1;
 break;
}
};
};
 ...
 if(parentObject.childsCount>=2)
{
 ...
 var nodeAskingCall="";
 if(!node){
 node=dhtmlObject.XMLLoader.getXMLTopNode("tag");
...
{
 if((node.childNodes[i].nodeType==1)&&(node.childNodes[i].tagName == "tag"))
{
 var nodx=node.childNodes[i];
 var name=nodx.getAttribute("label");
 ...
 if((!dhtmlObject.slowParse)||(dhtmlObject.waitUpdateXML))
{
 ...
 if(node.childNodes[i].childNodes[a].tagName=="tag"){
...
dhtmlXTreeObject.prototype._correctCheckStates=function(dhtmlObject){
 ...
 inp.checked=0;inp.src=this.imPath+this.checkArray[0];
...
{img.style.width="0px";img.style.height="0px";}
 ...
 td1.onclick=this.onRowClick;td2.ondblclick=this.onRowClick2;
...
dhtmlXTreeObject.prototype.checkNode=function(itemId,state){
 state=convertStringToBoolean(state);
 var sNode=this._globalIdStorageFind(itemId);
 if(!sNode)return;
