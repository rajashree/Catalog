 ...
 * {@inheritDoc}
 */
public final class OrderItemDataExceptionHandler  extends DataExceptionHandlerAdapter {
    ...
    public void onDataWriterException(final DataWriterException e) {
        ...
        if (e.getErrorId().equalsIgnoreCase("SINGLE_ROW")) {
            try {
                String path = OrderItemDataExceptionHandler.getErrorFilePath(e.getTableDefinition().getProperty("relativePath"),e.getTableDefinition().getProperty("startDate"));
            ...
            try {
                ...
                logger.warn("OrderDataExceptionHandler onDataWriterException() " + e.getMessage());
            }
            ...
            try {
                ...
                logger.warn("OrderItemDataExceptionHandler onDataWriterException() " + e.getMessage());
            }
        }
    ...
    static final String getErrorFilePath(String path, String startDate) {
        String loggerName = "error_feeds"+ StringUtils.getSimpleString(path.substring(0, (path.length())))+"_"+ DateUtils.TIMESTAMP_DATEFORMAT.format(DateUtils.getDate(startDate)).replaceAll(":", "_");
        return loggerName+"_errors.csv";
    }
}
