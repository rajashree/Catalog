 ...
 * {@inheritDoc}
 */
public final class OTGDataExceptionHandler extends DataExceptionHandlerAdapter {
    ...
    public void onDataWriterException(final DataWriterException e) {
        ...
        if (e.getErrorId().equalsIgnoreCase("SINGLE_ROW")) {
            try {
                String path = OTGDataExceptionHandler.getErrorFilePath(e.getTableDefinition().getProperty("relativePath"),e.getTableDefinition().getProperty("startDate"));
            ...
            try {
                ...
                logger.warn("OTGDataExceptionHandler onDataWriterException() " + e.getMessage());
            }
    ...
    static final String getErrorFilePath(String path, String startDate) {
        ...
        String loggerName = "/error_feeds/otg/"+ StringUtils.getSimpleString(path.substring(0, (path.length() - extension.length() - 1)))+"_"+ DateUtils.TIMESTAMP_DATEFORMAT.format(DateUtils.getDate(startDate)).replaceAll(":", "_");
