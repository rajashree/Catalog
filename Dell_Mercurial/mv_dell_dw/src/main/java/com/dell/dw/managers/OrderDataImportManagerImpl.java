 ...
 */
@Service
public class OrderDataImportManagerImpl  implements OrderDataImportManager {
    private static final Logger logger = LoggerFactory.getLogger(OrderDataImportManagerImpl.class);

    @Override
    @Transactional(readOnly = false)
    public List<DataSchedulerBatch> getUnprocessedBatches() {
        List<DataSchedulerBatch> list = dataSchedulerBatchRepository.getUnprocessedBatches(DataSource.DSConstants.ORDER);
        return list;
    }
    ...
    public void importData(DataSchedulerBatch record) {
        ...
        String loggerName = "com.dell.dw.dataimport."+ StringUtils
                .getSimpleString(lockedBatch.getFilePath())+"_"+ DateUtils.TIMESTAMP_DATEFORMAT.format(lockedBatch.getStartDate()).replaceAll(":", "_") ;
        ...
        try {
            ...
            File fileSchema = configurationService.getFileSystem().getFile("/config/order/data_import_config.xml", false, false);
            ...
            TableDefinition tableDefinition = schema.getDefinitionByDestination("com.dell.dw.persistence.domain.OrderItem");
            ...
            DataReader dataReader =  applicationContext.getBean("orderDataReader", DataReader.class);
