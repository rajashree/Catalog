 ...
 */
@Service(value = "otgDataImportManagerImpl")
public class OTGDataImportManagerImpl implements OTGDataImportManager {
    private static final Logger logger = LoggerFactory.getLogger(OTGDataImportManagerImpl.class);

    @Override
    @Transactional(readOnly = false)
    public List<DataSchedulerBatch> getUnprocessedBatches() {
        List<DataSchedulerBatch> list = dataSchedulerBatchRepository.getUnprocessedBatches(DataSource.DSConstants.OTG);
        return list;
    }
    ...
    public void importData(DataSchedulerBatch record) {
        ...
        String loggerName = "com.dell.dw.dataimport./otg/"+ StringUtils
                .getSimpleString(lockedBatch.getSrcFile().replace("csv",""))+ DateUtils.TIMESTAMP_DATEFORMAT.format(lockedBatch.getStartDate()).replaceAll(":", "_") ;
        ...
        try {
            ...
            File fileSchema = configurationService.getFileSystem().getFile("/config/otg/data_import_config.xml", false, false);
            ...
            TableDefinition tableDefinition = schema.getDefinitionByDestination("com.dell.dw.persistence.domain.OTGLead");
            ...
            tableDefinition.setProperty("relativePath", lockedBatch.getSrcFile());
            tableDefinition.setProperty("startDate",lockedBatch.getStartDate().toString());
            tableDefinition.setProperty("isPathAbsolute", "true");

            DataReader dataReader =  applicationContext.getBean("otgDataReader", DataReader.class);
