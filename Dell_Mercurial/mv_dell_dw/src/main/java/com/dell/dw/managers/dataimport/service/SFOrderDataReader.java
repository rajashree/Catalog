 ...
 */
public class SFOrderDataReader extends BaseDataAdapter implements DataReader {

    /**
     * default constructor.
     */
    public SFOrderDataReader() {
    }
            ...
            while ((row = reader.readNext()) != null) {
                try {
                    ...
                    for (i = 0; i < headersCount; i++) {
                        if(headers[i].equals("StoreID") || headers[i].equals("StoreId") || headers[i].equals("Merchantid") || headers[i].equals("MerchantId")){
                            String rowVal = row[i];
                            if(row[i].startsWith("{"))
                              rowVal = rowVal.replace("{","");
                            if(row[i].endsWith("}"))
                               rowVal = rowVal.replace("}","");
                            record.put(headers[i], rowVal);
