var CIUtils = {
    resetThumbnailImage: function() {
        ...
        var obj=document.getElementById('reset_img_flag');
        if(obj!=null){
            document.getElementById('reset_img_flag').value = 'true';
            $('#resetImageMsg').html('Upload the new image');
        }
        // Hide reset link
...
//            }
        ...
        startDateField.datepicker({
            ...
            buttonImage: basePath + 'images/cal.png',
...
var CIReview = {
    ...
    showReviewPopup: function(campaignItemID, itemID, valueString) {
        ...
        CIReview.reviewItems = values;
    },

    updateReviews: function() {
        if (CIReview.itemID != undefined) {
            CIReview.getItemReviews(CIReview.itemID, CIReview.campaignItemID, CIReview.reviewItems);
        }
    },
    ...
    addItemReviews: function(el, campaignItemID, itemID, reviewID) {
        ...
        if (el.checked) {
            // 2012/08/23 GB - removing limits on the number of reviews
    ...
    getItemReviews: function(itemID, campaignItemID, selectedValues) {
        var minStars = $("#starCount option:selected").val();
        if (minStars == undefined) minStars = CIReview.defaultMinStars;
        $.ajax({ url: 'reviews.json',
            data:{campaignItemID: campaignItemID, itemID: itemID, minStars: minStars},
            dataType:"json",
            success:function (response, status) {
                // Display the popup
...
var CItems = {
    ...
    addItems: function(el, itemID, elOverride, typeOverride) {
        ...
        if (checked) {
            ...
            if (CItems.selectedItems[type] != undefined) {
                CItems.selectedItems[type].push(itemID);
            } else {
                // If not initialize the object for the itemType
                CItems.selectedItems[type] = [];
                CItems.selectedItems[type].push(itemID);
            }
        } else {
            // Remove from array list
            ...
            window.console.log('BEFORE ', CItems.selectedItems[type]);
            for (var i = 0; i < CItems.selectedItems[type].length; i++) {
                if (CItems.selectedItems[type][i] != itemID) {
                    tmp.push(CItems.selectedItems[type][i]);
                }
            }
            CItems.selectedItems[type] = tmp;
            window.console.log('BEFORE ', CItems.selectedItems[type]);
        }
    },
    ...
    toggleAllItems: function(el, itemType) {
    	...
    	var toggleCheckboxes = function (checked) {
    		for (var i = 0; i < CItems.selectedItems[itemType].length; ++i)
    			$("#item_" + CItems.selectedItems[itemType][i]).attr("checked", checked);
    	};
    	
    	// guard against no initialization
    	if (CItems.selectedItems[itemType] == undefined) 
			CItems.selectedItems[itemType] = [];
    	...
    	if (el.checked == false) {
			...
			CItems.selectedItems[itemType] = CItems.allItems[itemType];			
    ...
    saveItems: function(campaignID) {
        ...
        // Add Product support
        if (CItems.selectedItems.product != undefined) {
            params.push("products=" + CItems.selectedItems.product.join(','));
        }
        // Add Event support
        if (CItems.selectedItems.event != undefined) {
            params.push("events=" + CItems.selectedItems.event.join(','));
        }
        // Add Document support
        if (CItems.selectedItems.document != undefined) {
            params.push("documents=" + CItems.selectedItems.document.join(','));
        }
        // Add Videos support
        if (CItems.selectedItems.video != undefined) {
            params.push("videos=" + CItems.selectedItems.video.join(','));
        }

        // Add Images support
        if (CItems.selectedItems.image != undefined) {
            params.push("images=" + CItems.selectedItems.image.join(','));
        }

        // Add Links support
        if (CItems.selectedItems.link != undefined) {
            params.push("links=" + CItems.selectedItems.link.join(','));
        }
    ...
    showFilterOptions: function(e) {
        ...
        if (v != undefined && v != '') {
            ...
            } else if (v == "document" || v == "link" || v == "image" || v == "video" ) {
                $('#event_doc_filters').show();
                $('#location_field').hide();
                // Re-Set other filter items
                $('#filtered_items').empty();
    ...
    applyFilter: function() {
        ...
        } else if (itemType == 'document' || itemType == "link" || itemType == "image" || itemType == "video" ) {
            params.keyword = $('#keyword').val();
            params.startDate = $('#startDate').val();
            params.endDate = $('#endDate').val();
        } else {
            ...
            success:function (response, status) {
                ...
                if (response.data.length > 0) {
                    ...
                    var headerHTML = "<td style='width: 12px;'>" + 
                    				   "<input type='checkbox' onchange='CItems.toggleAllItems(this,\"" + itemType + "\")' />" + 
                    				 "</td>";
                    if (itemType == 'product') {
                        ...
                        // title, url
                        headerHTML = headerHTML.concat("<td style='width: 170px;'>Title</td>");
                        headerHTML = headerHTML.concat("<td style='width: 110px;'>URL</td>");
                        headerHTML = headerHTML.concat("<td style='width: 110px;'>Image</td>");
                    } else if (itemType == 'link' || itemType == 'image'){
                        // title, url
                        headerHTML = headerHTML.concat("<td style='width: 170px;'>Title</td>");
                        headerHTML = headerHTML.concat("<td style='width: 110px;'>URL</td>");
                    }
                    ...
                    for (var i = 0; i < response.data.length; i++) {
                    	// add response data to allItems...
                   		if (CItems.allItems[itemType] == undefined)
                     		CItems.allItems[itemType] = [];
                    	CItems.allItems[itemType].push(response.data[i].id);
    ...
    itemRowBuilder: function(itemType, itemObj) {
        ...
        if (itemType == 'product') {
            ...
            rowHTML = "<tr id='row_" + itemObj.id + "' style='border-bottom: 1px solid;height: 25px;'>" +
                    ...
                    "</tr>";
        }else if (itemType == 'image' || itemType == 'link') {
            rowHTML = "<tr id='row_" + itemObj.id + "' style='border-bottom: 1px solid;height: 25px;'>" +
                "<td style='width: 12px;'><input onchange='CItems.addItems(this," + itemObj.id +
                ")' type='checkbox' name='items' id='item_" + itemObj.id + "' data-id='" + itemObj.id + "'></td>" +
                "<td style='width: 170px;'>" + itemObj.title + "</td>" +
                "<td style='width: 110px;'>" + itemObj.url + "</td>" +
                "</tr>";
        }else if (itemType == 'video') {
            rowHTML = "<tr id='row_" + itemObj.id + "' style='border-bottom: 1px solid;height: 25px;'>" +
                "<td style='width: 12px;'><input onchange='CItems.addItems(this," + itemObj.id +
                ...
                "<td style='width: 170px;'>" + itemObj.title + "</td>" +
                "<td style='width: 110px;'>" + itemObj.url + "</td>" +
                "<td style='width: 110px;'>" + itemObj.image + "</td>" +
                "</tr>";
        }
        return rowHTML;
    },
...
$(function() {
    ...
    $('#publishDate').datetimepicker({
        ...
        changeYear: true,
        onSelect: function (selectedDateTime) {
            var publish = $(this).datetimepicker('getDate');
            CIAutoComplete.validateFormFields();
        }
    });
});