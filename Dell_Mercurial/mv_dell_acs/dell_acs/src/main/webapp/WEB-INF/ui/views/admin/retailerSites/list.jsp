...
<head>
    <title>Retailer Site's list page</title>
...
<c:choose>
    <c:when test="${ empty retailer}">
        RETAILER NOT FOUND.
		<span>
		    <a href="<c:url value='/admin/retailerSites/add.do?retailerId=${retailer.id}'/>" class="gbutton">ADD SITE</a>
		</span>
    </c:when>
    <c:otherwise>
		${navCrumbs}
		<div id="tabs">
	        ...
	        <div id="inactive" style="border-top: 1px solid rgb(204, 204, 204);">
		        <table class="datatable">
		            ...
		            <tbody>
		            <c:forEach items="${retailerInactiveSites}" var="row">
		                <tr>
		                    <td style="width: 5%;">${row.id}</td>
		                    <td>${row.siteName}</td>
		                    <td>${row.retailer.description}</td>
		                    <td style="width: 8%;">${row.siteUrl}</td>
		                    <td style="text-align: center;">
		                        <!-- Edit Retailer Site -->
		                        <a style="padding-right: 10px;"
		                           href="<c:url value='/admin/retailerSites/edit.do?id=${row.id}'/>">Edit</a>
		
		                        <!-- View Jobs -->
		                        <a style="padding-right: 10px;"
		                           href="<c:url value='/admin/retailerSites/dataImport.do?id=${row.id}'/>">View Jobs</a>
		
		
		                        <a style="padding-right: 10px;"
		                           href="<c:url value='/admin/library/view.do?siteID=${row.id}'/>">Library</a>
		
		
		                        <!-- View Campaigns -->
		                        <a style="padding-right: 10px;"
		                           href="<c:url value='/admin/campaign/list.do?siteID=${row.id}'/>">View Campaigns</a>
		                    </td>
		                </tr>
		            </c:forEach>
		            </tbody>
			...
			<div id="active" style="border-top: 1px solid rgb(204, 204, 204);">
		        <table class="datatable">
		            ...
		            <tbody>
		            <c:forEach items="${retailerActiveSites}" var="row">
		                <tr>
		                    <td style="width: 5%;">${row.id}</td>
		                    <td>${row.siteName}</td>
		                    <td>${row.retailer.description}</td>
